...
class L1bparase:
    ...
    def func(self, row):
        ...
        value_1 = value_0.reshape((width, depth))  # 划分为1024行，10列的数组，每行代表10个波段，因此，从中取出第一列再转置就生成了第一个波段的第一行数据
        return value_1.T  # 切记，转置是以左上角为原点，逆时针转90°
    ...
    def read_write(self, file, HY1AL1Bfile_illustrate_file):
        ...
        try:
            ...
            if os.access(outfile, os.R_OK):
                ...
                scipy.misc.toimage(res, low=0).save(outfile1)
